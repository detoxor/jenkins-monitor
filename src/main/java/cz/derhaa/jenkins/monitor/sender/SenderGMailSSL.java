...
public class SenderGMailSSL extends SenderBase {
	...
	public static final String KEY = "gmail-ssl";
	...
	public void sendNotices(List<Notice> notices) {
		...
		for (Notice notice : notices) {
			...
			stringBuilder.append("[Jenkins]").append(" [").append(buildName).append("] ").append(message).append(" - ")
					.append(jenkinsUrl).append("/job/").append(buildName).append("/").append(lastBuildLabel)
					.append("\n\n");
		}
		try {
			mimeMsg.setText(stringBuilder.toString());
			Transport.send(mimeMsg);
			LOGGER.info("Message has been sended to " + addresses.toString() + ". Content of message: "
